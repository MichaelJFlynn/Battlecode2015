...
public class Mover {
	...
	public static void bugPath(MapLocation loc) throws GameActionException {
		...
		case HUGRIGHT:
			...
			if(canEndBugPath(d)) {
				...
				while(!traversable(myLoc.add(movingDirection))) {
	...
	private static boolean canEndBugPath(Direction dir) throws GameActionException {
		if(!traversable(rc.getLocation().add(dir))) {
			return false;
		}
		Direction movingDirection = wallDir;
		switch(bugPathState) {
	...
	public static boolean traversable(MapLocation m) throws GameActionException {
		RobotInfo rob = rc.senseRobotAtLocation(m);
		return rc.senseTerrainTile(m).isTraversable() && (rob == null || rob.type.isBuilding || rob.type == RobotType.HQ)
				&& !closeToTowers(m);
	}
	...
	private static void startBugPath(MapLocation loc) throws GameActionException {
		...
		while(!traversable(myLoc.add(hugLeftDir))) {
			hugLeftDir = hugLeftDir.rotateRight();
		}
		while(!traversable(myLoc.add(hugRightDir))) {
		...
		for(MapLocation loc : locs) {
			...
			for(RobotInfo enemy : enemies) {
				if(enemy.type != RobotType.MISSILE) { 
					...
					if(loc.distanceSquaredTo(enemy.location) <= enemy.type.attackRadiusSquared*8) {
						acceptable = false;
					}
				}
