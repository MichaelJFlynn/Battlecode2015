...
public class Skirmisher {
	...
	private static BattleInfo[] getBattleInfoForEachPossibleMove() throws GameActionException {
		...
		boolean advantage = 1.5*enemy_damage * enemy_health < friend_health * friend_damage;
		...
		for(Direction dir : directions) {
			...
			for(int i=0; i < enemies.length; i++) {
				...
				if(robot.type != RobotType.TOWER && robot.type != RobotType.HQ) { // these cases handled elsewhere
					if(!winning) {
	...
	public static void moveWithoutAttacking(MapLocation[] towers, RobotInfo[] enemies) throws GameActionException {
		...
		for(Direction dir : directions) {
			...
			if(goalLoc != null) { 
				receivable += m.distanceSquaredTo(goalLoc)/1000000;
			}
