...
public class Skirmisher {
	...
	private static BattleInfo[] getBattleInfoForEachPossibleMove() throws GameActionException {
		...
		friends = rc.senseNearbyRobots(myLoc, 24, myTeam);
		...
		double friend_health = 0;
		double friend_damage = 0; 
		...
		for(Direction dir : directions) {
			...
			for(int i=0; i < enemies.length; i++) {
				...
				if(2* enemy_health * enemy_damage > friend_health * friend_damage) {
			...
			if(2 *enemy_health * enemy_damage > friend_health * friend_damage) 
				receivable += RobotType.TOWER.attackPower * MessageBoard.towersNear(myLoc.add(dir));
			else {
				//receivable -= MessageBoard.towersNear(myLoc.add(dir));
			}
			...
			if(distanceToHQ <= RobotType.HQ.attackRadiusSquared + 2*Math.sqrt(RobotType.HQ.attackRadiusSquared) + 1 &&
					(2 *enemy_health * enemy_damage > friend_health * friend_damage) ) {
